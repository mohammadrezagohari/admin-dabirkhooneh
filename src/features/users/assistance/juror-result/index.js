...
import {
  ...
  getAssistanceJurorResult,
  getJurorCommentsList,
...
function JurorResult() {
  ...
  const showItemInfo = async (unique_key) => {
    const showResult = await getJurorCommentsList(
      unique_key,
      userToken,
      currentPage ,
      itemsPerPage
  ...
  console.log("response result showDetails: ", showDetails);
  ...
  // const { data } = useQuery(["categories-juror-res-list", userToken], () =>
    ...
    <>
      ...
      {isLoading ? (
        ...
        <TitleCard
          ...
          title={`${t('listOfComments')} ${showDetails?.data?.data[0]?.juror?.name}`}
          topMargin="mt-2"
        >
                  ...
                  currentPosts?.map((l, k) => {
                    return (
                      <tr key={k} className="cursor-pointer text-center">
                        <td>{++first}</td>
                        <td> <span  className=" threeDoteName">{l?.work?.title}</span></td>
                        ...
                        <td>
                           ...
                           <Link
                             to={`/app/work/info/${l?.work?.unique_key}/${l?.work?.tracking_code}`}
                             className="btn font-medium text-xs "
                           >
