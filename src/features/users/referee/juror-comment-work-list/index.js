...
import { getListAuthJurorCommentsWork } from "../../../../core/api/servises/general";
...
function JurorCommentWorkList() {
  const { t } = useTranslation();
  const { unique_key } = useParams();
  const { userToken } = useContext(AuthContext);
  const [showDetails, setShowDetails] = useState();
  // http://localhost:3000/app/juror-comment-work-list/A5t7lbUn
  ...
  return (
    <>
      <TitleCard
        title={` ${t('jurorCommentListPerWork')} ${
          showDetails?.more?.work_title === undefined
            ? " "
...
export default JurorCommentWorkList;
