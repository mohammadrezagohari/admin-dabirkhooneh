...
import { IoEyeOutline, IoReturnUpBack } from "react-icons/io5";
import { FaRegCommentAlt } from "react-icons/fa";
import {
  getArtsSearch,
  getJurorNewWorkList,
  getWorkSearch,
  postJurorCommentWork,
...
function RefereeList() {
  ...
  const storeItem = async (e) => {
    ...
    const createResult = await postJurorCommentWork(
      selected,
      formData,
      userToken
    )
                ...
                <table className="table w-full">
                  ...
                  <tbody>
                    {works?.data?.data?.length <= 0 ? (
                      <div className="ml-20 w-max flex justify-center text-md font-yekanReg  text-red-400">
                        {t("emptyMsg")}
                      </div>
                    ) : (
