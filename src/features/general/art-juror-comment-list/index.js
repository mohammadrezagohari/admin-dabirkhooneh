...
import { getListAuthJurorCommentsWork, getListJurorCommentsWork } from "../../../core/api/servises/general";
import ReactPaginate from "react-paginate";
import { FaChevronLeft, FaChevronRight } from "react-icons/fa";
import { getAdminDeleteWorkComment } from "../../../core/api/servises/users";
...
function ArtJurorCommentList() {
  const { t } = useTranslation();
  const { unique_key } = useParams();
  const { userToken,level } = useContext(AuthContext);
  ...
  const showItemInfo = async (unique_key) => {
    if (levels == 5) {
      ...
      const showResult = await getListAuthJurorCommentsWork(
    ...
    <>
      {isLoading ? (
        ...
        <TitleCard
          ...
          title={`   لیست نظرات داوران برای ${
            showDetails?.more === undefined
              ? " "
              : showDetails?.more?.work_title
          }    `}
          topMargin="mt-2"
        >
          {/* { unique_key }
        {showDetails?.data[0].description} */}
                  ...
                  currentPosts?.map((l, k) => {
                    return (
                      <tr key={k} className="cursor-pointer text-center">
                        <td>{++first}</td>
                        <td> <span  className=" threeDoteName">{l?.juror?.name}</span></td>
          ...
          {/* allData?.length >= per_page || apiCurrenPage== last_page */}
          {total > 0 ? (
            <>
              <ReactPaginate
...
export default ArtJurorCommentList;
