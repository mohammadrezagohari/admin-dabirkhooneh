...
function ArtGuestCommentList() { 
  ...
  const showItemInfo = async (unique_key) => {
    if (levels == "5") {
      const showResult = await getListAuthGuestCommentsWork(
        unique_key,
        userToken,
        apiCurrenPage,
        itemsPerPage
  ...
  return (
    <>
      <TitleCard
        ...
        title={`  لیست نظرات مهمان برای ${
          showDetails?.more === undefined ? " " : showDetails?.more?.work_title
        }   `}
        topMargin="mt-2"
      >
        {/* { unique_key }
                ...
                currentPosts?.map((l, k) => {
                  return (
                    <tr key={k} className="cursor-pointer text-center">
                      <td>{++first}</td>
                      <td>
                        <span className=" threeDote">{l?.juror?.name}</span>
                      </td>
...
export default ArtGuestCommentList;
